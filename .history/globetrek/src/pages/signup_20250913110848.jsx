...
// Improved email existence check with better error handling and user experience
const checkEmailExists = async (email) => {
  try {
    // Use a more realistic redirect URL or get from environment
    ...
    // If no error and we get a response, email likely exists
    if (!error) {
    ...
    if (errorMessage.includes('rate limit') || errorCode === 'rate_limit_exceeded') {
      ...
      // Return null to indicate we couldn't determine existence
      return null;
    }
    ...
    return null; // Indicate we couldn't determine existence
    ...
    if (err.name === 'NetworkError' || err.message.includes('fetch')) {
      return null; // Network issue, can't determine
    }
    
    return null; // Assume we can't determine existence on unexpected errors
  }
};

// Enhanced email validation with existence check integration
const handleEmailChangeWithExistenceCheck = async (e) => {
  const value = e.target.value;
  setEmail(value);
  ...
  if (value.trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value.trim())) {
    ...
    // Store timeout ID to clear if user types again quickly
    setEmailCheckTimeout(timeoutId);
  }
};

// Usage in the signup form with better UX integration
const handleSignupWithEmailCheck = async (e) => {
  e.preventDefault();
...
// Additional state needed for debouncing
