...
export default function Signup() {
  ...
  const checkEmailExists = async (email) => {
    try {
      ...
      // If we get any other error, email likely exists
      if (error?.message === 'Invalid login credentials') {
        return false; // Email doesn't exist
      } else {
        return true; // Email exists or other issue
      }
    } catch (err) {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (emailExists) {
        throw new Error("This email is already registered. Please try signing in instead or check your email for a confirmation link.");
      }
      ...
      // Step 3: Insert into users table (this happens after email is sent)
      ...
      if (insertError) {
        ...
        // This is tricky - the auth user is already created and email sent
        // We can't easily undo this, so we'll just show an error
        // The user will exist in auth but not in users table
