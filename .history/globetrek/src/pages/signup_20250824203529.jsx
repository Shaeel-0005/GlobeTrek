...
export default function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      if (data?.user) {
        // 2. Always insert into users table
        const { error: insertError } = await supabase.from("users").insert({
          id: data.user.id, // link to auth.users
          name: name,       // <-- force your provided name
          email: email,
        });
        ...
        if (data.session) {
          // If confirmation emails are OFF
          navigate("/dashboard", { state: { userName: name } });
        } else {
          // If confirmation emails are ON
          setSuccess(
      ...
      if (err.message.includes("Password should be at least 6 characters")) {
        ...
        friendlyError =
          "This email is already registered. Please check your inbox for confirmation or try signing in.";
