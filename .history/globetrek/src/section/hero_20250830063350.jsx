...
const Hero = () => {
  ...
  const { scrollYProgress } = useScroll({
    target: containerRef,
    offset: ["start end", "end start"], // track scroll inside Hero section
  });

  // Parallax mapping (images move slower than text)
  ...
  return (
    <div className="scroll-smooth bg-gradient-to-b from-white via-gray-50 to-gray-100 text-[#2E2E2E]">
      ...
      <section
        ref={containerRef}
        className="relative h-[350vh] flex flex-col items-center"
      >
        {/* Sticky Heading */}
        <motion.div
          className="sticky top-28 z-20 flex items-center justify-center h-40"
          initial={{ scale: 1 }}
          whileInView={{ scale: 0.9, opacity: 0.95 }}
...
export default Hero;import React, { useRef } from "react";
import { Navbar } from "../components/index";
import { Hero01, Hero02, Hero03 } from "../assets/index";
import { motion, useScroll, useTransform } from "framer-motion";
